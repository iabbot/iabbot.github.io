...
function PopulatePanel() {
    ...
    NewButton("iab!edge", true, "iab!edge {unorderedParameters}", false, "Generates an edgemap. If no options are specified, these values are used: Bias 0, Color FFFFFF, Multiplier 1, Search-Radius 1, Tolerance 30, AlphaMode 1. If no image is specified, iab will search up to 32 messages above the calling message to find an applicable image. Alpha Modes: 0 means that pixels defined as edges have 255 alpha, and other pixels have 0 alpha. 1 uses the image's old alpha value, and 2 uses the contrast of the pixels to determine the alpha.", ["Image: The image to create an edgemap from. Can be linked, attached (iab has a tendency to fail to download attached images. If this happens, simply right click the image -> \"Copy Link\" -> send link with edge command (no switch, just slap it on the end. iab will figure it out.), or linked from MEGA with -mega (link)", "-bias #: Add or subtract this amount from the final alpha amounts.", "-color hex#: The color of the edgemap.", "-hue: Uses the color of the image instead of the color specified, if any.", "-multiplier #: Multiply the alpha value by this number (before bias)", "-radius #: Use this radius to search for edges.", "-tolerance #: The amount a channel must differ to be considered an edge.", "-alphamode #: The mode of alpha calculation to use. See the description."]);
